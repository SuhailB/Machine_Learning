...
def Cost(X,y,w,b,C):
    ...
    sum1 = (1-y*(X @ w.T + b)).clip(min=0) 
    sum2 = np.power(w,2)
    return (np.sum(sum2)/2)+C*np.sum(sum1)

print("I_Cost",Cost(X,y,w,b,10))
...
w_,b_,c,count = Mini_Batch_SVM(X,y,w,b,C,alpha,epsilon)
print("F_cost",Cost(X,y,w_,b_,C))
#print(w_,b_)
#print("F_Loss",hinge_loss(w_,X,y,b_))
