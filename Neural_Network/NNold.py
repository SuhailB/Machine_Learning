...
def Loss_Function(X,Y,W1,W2,Y_):
    ...
    return sum1+sum2
H,Y_,Z1 = Forward_Propagation(X,Y,W1,W2)
...
W1_,W2_,Y_ = Gradient_Descent(X,Y,W1,W2)
print(Loss_Function(X,Y,W1_,W2_,Y_))

num = np.where(Y[0]==max(Y[0]))[0][0]
print(num)
...
W1_test = (pd.read_csv('data\W1_grad.csv', header=None)).values
W2_test = (pd.read_csv('data\W2_grad.csv', header=None)).values
